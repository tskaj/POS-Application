...
class _AddPaymentPageState extends State<AddPaymentPage> {
  ...
  List<AccountOfSubHead> _allAccounts =
      []; // For both Cash and Bank mode - direct head selection
  ...
  Future<void> _submitForm() async {
    ...
    if (_selectedPaymentMode == PaymentMode.bank) {
      if (_selectedCoaId == null) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Please select all chart of account levels'),
            backgroundColor: Colors.red,
          ),
        );
        return;
    ...
    try {
      ...
      // Required fields (API expects these exact keys):
      // - transaction_type_id: 9 (Expense)
      // - payment_mode_id: 1 for Cash, 2 for Bank
      // - expense_category_id: comes from selected COA (fallback to 34)
      final expenseData = {
        'date': DateFormat('yyyy-MM-dd').format(_selectedDate),
        // API-required fields
        'transaction_type_id': 9,
        'payment_mode_id': _selectedPaymentMode == PaymentMode.cash ? 1 : 2,
        'expense_category_id': _selectedCoaId ?? 34,

        // Keep older/coexisting fields for backward compatibility where useful
        'coas_id': _selectedCoaId,
        'ccoas_id': _selectedPaymentMode == PaymentMode.bank
      ...
      if (response['status'] == true) {
        ...
        // Clear cached payouts so callers reload fresh data from the API
        final financeProvider = Provider.of<FinanceProvider>(
          context,
          listen: false,
        );
        financeProvider.clearPayouts();

        Navigator.of(context).pop(true); // Return true to indicate success
      } else {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(response['message'] ?? 'Failed to create expense'),
            backgroundColor: Colors.red,
          ),
        );
      }
          ...
          children: [
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    TextFormField(
                      ...
                      decoration: _buildCleanInputDecoration(
                        'Description',
                        hint: 'Additional details about the payment',
                        prefixIcon: Icon(
                          Icons.info_outline,
                          color: Color(0xFF0D1845),
                        ),
            ...
            Container(
              ...
              child: ElevatedButton(
                ...
                style: ElevatedButton.styleFrom(
                  backgroundColor: _selectedPaymentMode == PaymentMode.cash
                      ? Color(0xFF28A745)
                      : Color(0xFF007BFF),
                  foregroundColor: Colors.white,
                  padding: const EdgeInsets.symmetric(vertical: 16),
